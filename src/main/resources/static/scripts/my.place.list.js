...
function renderPlace(data) {
    ...
    if (data.length >= 1) {
        ...
        data.forEach((item) => {
            ...
            <a class="group mt-2 block relative overflow-hidden rounded-lg border border-gray-400" href="/mypage/rental/view/${item.id}">
                <div class="absolute w-full h-full bg-gray-300 opacity-0 group-hover:opacity-50 transition left-0 top-0"></div>
                <img class="min-h-68 w-full object-cover bg-gray-100" src="/images/place_thumbnail/${item.thumbnail}" alt="">
        ...
        result = `<div class="flex sm:col-span-2 justify-center items-center">
